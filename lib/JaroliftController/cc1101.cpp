...
uint8_t gpio_sck = 18;
uint8_t gpio_miso = 19;
uint8_t gpio_mosi = 23;
uint8_t gpio_cs = 5;
uint8_t gpio_gdo0 = 21;

/**
 * Macros
 */
// Select (SPI) CC1101
#define cc1101_Select() SPI.begin(gpio_sck, gpio_miso, gpio_mosi, gpio_cs)
// Deselect (SPI) CC1101
...
//  Get GDO0 pin state
#define getGDO0state() digitalRead(gpio_gdo0)
 ...
 * set GPIO PIN´s
 */
void CC1101::setGPIO(int8_t sck, int8_t miso, int8_t mosi, int8_t ss, int8_t gdo0) {
  gpio_sck = sck;
  gpio_miso = miso;
  gpio_mosi = mosi;
  gpio_cs = ss;
...
bool CC1101::init(uint8_t freq) {
  ...
  pinMode(gpio_gdo0, INPUT); // Config GDO0 as input
 ...
 */
void CC1101::setSyncWord(byte *sync) { CC1101::setSyncWord(sync[0], sync[1]); }
